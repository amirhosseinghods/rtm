...
def _rsi_proj(sig):
    ...
    if p:
        if p.get("dir_val") == 0:
            out.append("• پیش‌بینیِ جهت: **بدونِ سوگیریِ مطمئن** — مدل در این لحظه لبهٔ جهتیِ کافی "
                       "نمی‌بیند و آگاهانه از پیش‌بینی پرهیز می‌کند (صبر؛ دقتِ سیستم وقتی پیش‌بینی می‌کند بالاتر است).")
        else:
